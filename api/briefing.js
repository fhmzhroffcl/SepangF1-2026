...
export default async function handler(req,res){
 ...
 try{
  ...
  const data={text:text.trim().slice(0,2200),date:row?.date||null,generatedAt:new Date().toISOString(),source:topic==='circuit'?'Formula 1 circuit guide':'MET Malaysia / data.gov.my',model:result.model||MODEL};
  ...
  return res.status(503).json({error:'AI briefing is temporarily unavailable. Use the official forecast below and try again later.'});
