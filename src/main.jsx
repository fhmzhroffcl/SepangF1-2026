...
function saveCalendar(){const clean=s=>s.replace(/[-:]/g,'');const lines=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//Sepang 26//EN',...sessions.flatMap(s=>['BEGIN:VEVENT',`UID:sepang26-${s.start}@sepang26`,`DTSTAMP:${clean(new Date().toISOString().replace(/\.\d+Z$/,'Z'))}`,`DTSTART:${clean(s.start)}`,`DTEND:${clean(s.end)}`,`SUMMARY:${s.name} — Bahrain Grand Prix in Malaysia`,'LOCATION:Sepang International Circuit','END:VEVENT']),'END:VCALENDAR'];const url=URL.createObjectURL(new Blob([lines.join('\r\n')],{type:'text/calendar'})),a=document.createElement('a');a.href=url;a.download='sepang-26-weekend.ics';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}

function RaceHub({now}){const next=sessions.find(s=>Date.parse(s.end)>now),count=parts(next?Math.max(0,Math.floor((Date.parse(next.start)-now)/1000)):0);return <section id="race-hub" className="race-hub"><div className="hub-copy"><h1>Race weekend,<br/>under control.</h1><p>SEPANG INTERNATIONAL CIRCUIT · 2—4 OCT 2026</p><div className="next-session"><span>NEXT SESSION</span><h2>{next?.name||'Weekend complete'}</h2>{next&&<p>{new Date(next.start).toLocaleDateString('en-GB',{timeZone:'Asia/Kuala_Lumpur',weekday:'short',day:'numeric',month:'short'}).toUpperCase()} · {timeMY(next.start)} MYT</p>}<div className="countdown" aria-live="off">{Object.entries(count).map(([k,v])=><div key={k}><strong>{String(v).padStart(2,'0')}</strong><small>{k}</small></div>)}</div><div className="hero-actions"><a href="#circuit">View circuit <ArrowRight/></a><a href="#fan-card">Make fan card <ArrowRight/></a></div></div></div><div className="track-ghost"><img src={TRACK_IMAGE} alt=""/><span>SAME GROUND.<br/>A BRIGHTER WEEKEND.</span></div><WeatherPanel/><div className="session-rail">{sessions.map(s=><div key={s.name} className={s===next?'active':''}><i/><span>{sessionShort[s.name]||s.name}</span><small>{new Date(s.start).toLocaleDateString('en-GB',{timeZone:'Asia/Kuala_Lumpur',weekday:'short',day:'numeric',month:'short'})}<b>{timeMY(s.start)} MYT</b></small></div>)}<button onClick={saveCalendar}><CalendarPlus/>Calendar</button></div></section>}
...
function App(){const [now,setNow]=useState(Date.now()),[drivers,setDrivers]=useState(SNAPSHOT.drivers);useEffect(()=>{const t=setInterval(()=>setNow(Date.now()),1000);return()=>clearInterval(t)},[]);return <><header><a className="brand" href="#race-hub"><b>SEPANG</b><strong>26</strong></a><nav><a href="#race-hub">Race Hub</a><a href="#circuit">Circuit</a><a href="#grid">Grid</a><a href="#fan-card">Fan Card</a></nav><span>KUALA LUMPUR · MYT</span></header><main><RaceHub now={now}/><CircuitExplorer/><Standings onDrivers={setDrivers}/><FanStudio drivers={drivers}/><WeekendGuide/><Story/></main><footer><b>SEPANG 26</b><p>Independent fan companion · not affiliated with Formula 1 or the event organisers.</p><p>Schedule and circuit facts verified {VERIFIED}. Weather: MET Malaysia / data.gov.my. Map: © OpenStreetMap contributors. Standings: Jolpica.</p></footer></>}
