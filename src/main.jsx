...
function UpdateNotice(){const [needed,setNeeded]=useState(false);useEffect(()=>{const notify=()=>setNeeded(true);window.addEventListener('vite:preloadError',notify);return()=>window.removeEventListener('vite:preloadError',notify);},[]);return needed?<aside className="app-update-notice" role="alert"><span>A newer version is ready. Refresh to restore downloads. Your saved cards stay on this device.</span><button onClick={()=>window.location.reload()}>Refresh app</button></aside>:null;}
function App(){const [drivers,setDrivers]=useState(SNAPSHOT.drivers);return <><a className="skip-link" href="#race-hub">Skip to content</a><header><a className="brand" href="#race-hub"><b>SEPANG</b><strong>26</strong></a><nav aria-label="Main navigation"><a href="#race-hub">Race Hub</a><a href="#grid">Tracker</a><a href="#circuit">Circuit</a><a href="#my-weekend">My Weekend</a><a href="#fan-card">Fan Card</a></nav><SoundControl/></header><UpdateNotice/><main><RaceHub/><figure className="sepang-photo-band"><img loading="lazy" src="https://media.formula1.com/image/upload/t_16by9Centre/c_lfill%2Cw_3392/q_auto/v1740000001/fom-website/2026/Miscellaneous/SD-2017-Malaysia-Saturday-154_1981744%20169.webp" alt="Fans and Formula 1 at Sepang in 2017"/><figcaption><small>BACK WHERE WE BELONG</small><strong>Same ground.<br/>A new chapter.</strong><span>Sepang 2017 archive · Formula 1</span></figcaption></figure><SessionTimeline saveCalendar={saveCalendar}/><Standings onDrivers={setDrivers}/><MyWeekend saveCalendar={saveCalendar}/><CircuitExplorer/><FanStudio drivers={drivers}/><Tickets/><WeekendGuide/><Story/></main><footer><b>SEPANG 26</b><p>Independent fan companion · not affiliated with Formula 1 or the event organisers.</p><p>Schedule and circuit facts checked {VERIFIED}. <a href={OFFICIAL} target="_blank" rel="noreferrer">Official event updates ↗</a><br/>Weather: MET Malaysia / data.gov.my. Results: Jolpica.</p></footer></>}
