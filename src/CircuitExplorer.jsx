...
export default function CircuitExplorer(){
 ...
 const move=step=>setSelected(c=>corners[(c.n-1+step+15)%15]);
 ...
 return <section id="circuit" className="circuit-section">
  ...
  <div className="circuit-workspace">
   <div className="accurate-track">
    <div className="circuit-art" onTouchStart={e=>{e.currentTarget.dataset.touchStart=e.changedTouches[0].clientX;}} onTouchEnd={e=>{const delta=e.changedTouches[0].clientX-Number(e.currentTarget.dataset.touchStart);if(Math.abs(delta)>55){setMode('corners');move(delta<0?1:-1);}}}>
    ...
    {mode==='corners'?<div className="corner-picker" aria-label="Choose a corner"><button aria-label="Previous corner" onClick={()=>move(-1)}>←</button>{corners.map(c=><button key={c.n} aria-label={`Turn ${c.n}`} aria-pressed={selected.n===c.n} className={selected.n===c.n?'active':''} onClick={()=>{setSelected(c);}}>{c.n}</button>)}<button aria-label="Next corner" onClick={()=>move(1)}>→</button></div>:<div className="sector-picker">{sectors.map(s=><button key={s.n} aria-pressed={sector===s.n} onClick={()=>{setSector(s.n);}} className={sector===s.n?'active':''} style={{'--sector':s.color}}><i/>Sector {s.n}<small>Turns {s.turns}</small></button>)}</div>}
   </div>
  ...
  <p className="tour-caption">Swipe the circuit, scroll the corner strip or pick a glowing number. Your lap, your pace.</p><AIBriefing topic="circuit"/><div className="track-facts"><span><b>5.543 KM</b>Full lap</span><span><b>56</b>Race laps</span><span><b>310.408 KM</b>Race distance</span><span><b>1:34.080</b>Race lap record · Vettel, 2017</span></div>
 </section>;
}
